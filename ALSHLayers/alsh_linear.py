...
class ALSHLinear(nn.Module):
    def __init__(self, input_size, num_nodes, hf, table_size, m, P, Q):
        ...
        self._hash = hf # should be a class that defines __call__
    ...
    def _build_alsh_table(self):
        ...
        for i, row in enumerate(self.weight):
            hash_out = self._hash(self.P(row, self.m))
            index = (torch.abs(hash_out % self._table_size)).long().cuda()
    ...
    def rehash(self):
        ...
        hash_out = self._hash(self.Q(unit_ave_input, self.m))
        ...
        for row in rows:
            hash_out2 = self._hash(self.P(self.weight[row][0], self.m))
            hash_out2.fmod_(self._table_size)
            hash_out2.abs_()
    ...
    def forward(self, x, mode):
        ...
        out = ALSHOp.apply(x, self.weight, self.Q, self.m, self._hash,
                           self.table, self._table_size).cuda()
