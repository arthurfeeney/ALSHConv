...
def _append_norm_powers(x, m, device=torch.device('cuda')):
    ...
    x_n = torch.norm(x)
    powers = fill_powers_of_norm(x_n, m, sub=False, device=device)
    return torch.cat((x, powers))

def append_norm_powers(x, m, U=0, check=True, kernel_size=None,
    ...
    if x.dim() == 1:
        ...
        powers = fill_powers_of_norms(x_ns, m, sub=False, 
